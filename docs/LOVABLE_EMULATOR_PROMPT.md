...
| Emulator Sync API Key | `VITE_EMULATOR_SYNC_API_KEY` | Key for `emulator-sync` endpoint |
| Device Ingest API Key | `VITE_DEVICE_INGEST_API_KEY` | Key for `ingest-readings` endpoint |
| Org State Sync API Key | `VITE_ORG_STATE_SYNC_API_KEY` | Key for `org-state-api` endpoint |
...
**LiFeS2 AA (2-cell pack — most common):**
| Voltage | % |
|---------|---|
| 3.60V | 100% |
| 3.20V | 80% |
...
## 19. Implementation Checklist for Lovable

Use this checklist when building the export feature:
...
- [ ] **Send Readings function** that builds the `ingest-readings` payload
  ...
  - Sends `temperature_unit: "C"` (most common)
