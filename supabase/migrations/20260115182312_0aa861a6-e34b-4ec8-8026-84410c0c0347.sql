-- Fix: Remove invalid g.deleted_at filter from gateways query
-- The gateways table does NOT have a deleted_at column, causing RPC failure
...
BEGIN
  ...
  -- Query all ACTIVE sensors (exclude archived sensors)
  SELECT COALESCE(jsonb_agg(jsonb_build_object(
    'id', ls.id,
    'name', ls.name,
    'dev_eui', ls.dev_eui,
  ...
  -- Query all gateways (NO deleted_at filter - column doesn't exist on gateways table)
  SELECT COALESCE(jsonb_agg(jsonb_build_object(
    'id', g.id,
    'name', g.name,
