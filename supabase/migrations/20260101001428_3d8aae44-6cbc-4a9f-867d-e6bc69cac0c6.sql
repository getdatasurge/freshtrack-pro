-- Update emit_user_sync function to include TTN connection data
...
BEGIN
  ...
  IF NEW.organization_id IS NOT NULL THEN
    ...
    -- Fetch TTN connection data for this org (same pattern as sites)
    SELECT jsonb_build_object(
      ...
      'webhook_secret_last4', tc.ttn_webhook_secret_last4,
      'updated_at', tc.updated_at
    )
    INTO v_ttn_connection
    FROM public.ttn_connections tc
    WHERE tc.organization_id = NEW.organization_id;
  ELSE
    v_user_sites := '[]'::jsonb;
    v_default_site_id := NULL;
    v_ttn_connection := NULL;
  END IF;

  -- Build the payload with site membership data and TTN connection
  payload := jsonb_build_object(
    ...
    'ttn', COALESCE(v_ttn_connection, jsonb_build_object('enabled', false, 'provisioning_status', 'not_started'))
  );
