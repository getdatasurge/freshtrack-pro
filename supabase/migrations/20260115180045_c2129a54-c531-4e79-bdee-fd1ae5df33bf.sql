-- Fix: Filter archived sensors and gateways from org-state-api payload
-- This prevents archived (soft-deleted) entities from being sent to the emulator
...
BEGIN
  ...
  -- Query all ACTIVE sensors (CRITICAL FIX: exclude archived sensors)
  SELECT COALESCE(jsonb_agg(jsonb_build_object(
    'id', ls.id,
    'name', ls.name,
    'dev_eui', ls.dev_eui,
  ...
  -- Query all ACTIVE gateways (exclude soft-deleted)
  SELECT COALESCE(jsonb_agg(jsonb_build_object(
    'id', g.id,
    'name', g.name,
